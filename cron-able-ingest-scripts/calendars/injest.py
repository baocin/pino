...
class CalendarInjest:
    ...
    def fetch_calendar_events(self, account):
        try:
            ...
            print(f"Fetching calendar events for {account['username']}")
            print(f"Calendars: {([calendar.url for calendar in calendars])}")
            print(f"Calendars: {([calendar.name for calendar in calendars])}")
            ...
            for calendar in calendars:
                ...
                for event in events:
                    ...
                    try:
                        ...
                        logging.info(f"Calendar data: {calendar_data}")
                    ...
                    print(f"Inserting calendar data for {event_id}")
    ...
    def insert_calendar_data(self, calendar_data):
        sql = """
        INSERT INTO calendar_events (event_id, summary, start_time, end_time, description, location, pull_id, gps_point, embedding)
    ...
    def event_exists(self, event_id):
        sql = "SELECT 1 FROM calendar_events WHERE event_id = %s"
        cursor = self.db.cursor()
